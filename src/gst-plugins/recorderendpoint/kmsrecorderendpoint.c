...
recv_sample (GstAppSink * appsink, gpointer user_data)
{
  ...
  // initial timestamp of 0 (0:00:00.000).
  {
    ...
    GstClockTime time_offset = self->priv->paused_time;
    ...
    if (self->priv->gaps_fix == KMS_RECORDER_GAPS_FIX_GENPTS) {
      ...
      time_offset += base_time->audio_gaps;
    }

    if (GST_CLOCK_TIME_IS_VALID (base_time->pts)
        && GST_BUFFER_PTS_IS_VALID (buffer)) {
      time_offset += base_time->pts;

      // PTS -= time_offset, but preventing underflows.
      if (GST_BUFFER_PTS (buffer) > time_offset) {
        GST_BUFFER_PTS (buffer) -= time_offset;
      } else {
    ...
    if (GST_CLOCK_TIME_IS_VALID (base_time->dts)
        && GST_BUFFER_DTS_IS_VALID (buffer)) {
      time_offset += base_time->dts;

      // DTS -= time_offset, but preventing underflows.
      if (GST_BUFFER_DTS (buffer) > time_offset) {
        GST_BUFFER_DTS (buffer) -= time_offset;
      } else {
